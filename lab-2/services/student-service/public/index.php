...
try {
    ...
    if ($method === 'POST' && $uri === '/api/students') {
        ...
        if (empty($body['name']) || empty($body['email'])) {
            ...
            echo json_encode(['error' => 'VALIDATION_ERROR', 'message' => 'Name and email are required']);
            exit;
        }
        ...
        if (!filter_var($body['email'], FILTER_VALIDATE_EMAIL)) {
            http_response_code(400);
            echo json_encode(['error' => 'VALIDATION_ERROR', 'message' => 'Invalid email format']);
            exit;
        }
        ...
        if ($stmt->fetch()) {
            ...
            echo json_encode(['error' => 'DUPLICATE_EMAIL', 'message' => 'A student with this email already exists']);
            exit;
        }
    ...
    if (preg_match('#^/api/students/(\d+)$#', $uri, $matches)) {
        ...
        if ($method === 'GET') {
            ...
            if (!$student) {
                ...
                echo json_encode(['error' => 'STUDENT_NOT_FOUND', 'message' => 'Student with ID ' . $id . ' not found']);
                exit;
            }
