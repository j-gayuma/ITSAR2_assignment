...
try {
    ...
    if ($method === 'POST' && $uri === '/api/courses') {
        if (empty($body['name']) || empty($body['code'])) {
            ...
            echo json_encode(['error' => 'VALIDATION_ERROR', 'message' => 'Name and code are required']);
            exit;
        }
        ...
        if ($stmt->fetch()) {
            ...
            echo json_encode(['error' => 'DUPLICATE_CODE', 'message' => 'A course with this code already exists']);
            exit;
        }
    ...
    if (preg_match('#^/api/courses/(\d+)$#', $uri, $matches)) {
        ...
        if ($method === 'GET') {
            ...
            if (!$course) {
                ...
                echo json_encode(['error' => 'COURSE_NOT_FOUND', 'message' => 'Course with ID ' . $id . ' not found']);
                exit;
            }
